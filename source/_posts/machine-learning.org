...
PCA 有 9-10 种不同的理解方法。此处使用点到投影的距离的平方和最小。
投影应尽可能的分散，使得方差尽可能的大。
...
+ 应用到文本数据，从而剔除噪声，更好的比较文件之间的相似性。如单词 "study" 和 "learn" 如果单纯计算余弦值，两者的相关系数
  为 0 ，但若将两者投影到特征向量上，两者的相关系数将变为正值，所有相似的文本将变得相关。

*** Independent Components Analysis

独立成分分析(ICA)和主成分分析一样也是找到一组新的基来表示数据，但目的不同。ICA 用于找到样本中的相互独立的组件。
